...
import { Menu, X, Eye, Brain, Users, Mail, TestTube, ArrowRight, Upload, LogIn, LogOut, User, Settings, Contrast, Type } from "lucide-react"
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? "backdrop-blur-md border-b border-white/10 shadow-2xl shadow-black/20" : "bg-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center space-x-2 group">
            ...
            <span className="font-bold text-xl bg-gradient-to-r from-blue-400 via-purple-400 to-teal-400 bg-clip-text text-transparent group-hover:from-teal-400 group-hover:via-blue-400 group-hover:to-purple-400 transition-all duration-500">
              {"Dristi AI"}
            </span>
          </Link>
                ...
                <motion.div key={item.href} className="relative">
                  <Link
                    ...
                    className={`relative flex items-center space-x-2 text-sm font-medium transition-all duration-300 px-3 py-2 rounded-lg group ${
                      isActive ? "text-blue-400" : "text-gray-300 hover:text-white hover:bg-white/5"
                    }`}
                  >
                    <Icon
